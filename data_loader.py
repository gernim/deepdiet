...
class DataPathManager:
    ...
    def _get_dish_file_paths(self, dish_id):
        ...
        # Top-down imagery (if needed)
        top_down_paths = [
            f"imagery/realsense_top_down/{dish_folder}/rgb.png",
            f"imagery/realsense_top_down/{dish_folder}/depth_color.png",
            f"imagery/realsense_top_down/{dish_folder}/depth_raw.png",
        ]
        
        files_to_download.extend(overhead_paths)
        files_to_download.extend(top_down_paths)
...
def setup_data(force_download=False, use_splits=True, 
               include_overhead=True, include_top_down=True, include_side_angles=True):
    ...
    Args:
        ...
        include_overhead: Download overhead RGB and depth images
        include_top_down: Download top-down RGB and depth images
        include_side_angles: Download side angle sampled frames (~16 frames per dish)
    """
    manager = DataPathManager(
        include_overhead=include_overhead,
        include_top_down=include_top_down,
        include_side_angles=include_side_angles
    )
